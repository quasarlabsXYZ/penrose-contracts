...
func __setup__{bitwise_ptr : BitwiseBuiltin*, syscall_ptr: felt*, range_check_ptr}():
    ...
    %{ print("__setup__ caller: " + str(ids.caller)) %}
...
func test_starting_price{syscall_ptr: felt*, pedersen_ptr: HashBuiltin*, range_check_ptr}():
    ...
    assert_eq(price, 230584300921369395200)
    return ()
end
...
func test_price_decay_above_target_rate{syscall_ptr: felt*, pedersen_ptr: HashBuiltin*, range_check_ptr}():
    ...
    %{ roll(50, ids.penrose) %}
    let block_num: felt = Penrose.blockNumber(penrose)
...
func test_price_decay_below_target_rate{syscall_ptr: felt*, pedersen_ptr: HashBuiltin*, range_check_ptr}():
    ...
    let final_price: felt = Penrose.getQuote(contract_address=penrose)
     %{ print("initial: " + str(ids.initial_price) + " | final: " + str(ids.final_price)) %}
...
func test_price_increase_above_target_rate{syscall_ptr: felt*, pedersen_ptr: HashBuiltin*, range_check_ptr}():
    ...
    %{ ids.penrose = context.penrose %}
    %{ roll(0, ids.penrose) %}
    let gnspP: felt = Penrose.getNextPurchaseStartingPrice(penrose)
    %{ print("GNSP PREMINT: " + str(ids.gnspP)) %}
    Penrose.mint(contract_address=penrose)
    let gnsp: felt = Penrose.getNextPurchaseStartingPrice(penrose)
    %{ print("GNSP BLOCK 0: " + str(ids.gnsp)) %}
    %{ roll(1, ids.penrose) %}
    let gnsp1: felt = Penrose.getNextPurchaseStartingPrice(penrose)
    %{ print("GNSP BLOCK 1: " + str(ids.gnsp1)) %}
    let initial_price: felt = Penrose.getQuote(contract_address=penrose)
    Penrose.mint(contract_address=penrose)
    Penrose.mint(contract_address=penrose)
    %{ roll(4, ids.penrose) %}
...
func test_mint{syscall_ptr: felt*, pedersen_ptr: HashBuiltin*, range_check_ptr}():
    ...
    %{ print("pre caller: " + str(ids.pre_balance_of1.low) + " | pre tsy: " + str(ids.pre_balance_of2.low)) %}
    %{ print("pre_allowance: " + str(ids.pre_allowance.low)) %}
    ...
    # IERC20.approve(ETH, caller, Uint256(0, 0))
    ...
    %{ print("post caller: " + str(ids.post_balance_of1.low) + " | post tsy: " + str(ids.post_balance_of2.low)) %}
    %{ print("post_allowance: " + str(ids.post_allowance.low)) %}
    %{ 
        close_user_1()
        close_user_1b()
        close_user_1c()
