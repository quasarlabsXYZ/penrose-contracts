...
# <constuctor>--------------------------------------------------------------------------------------------------------------------
    ...
    def __init__(self):
        ...
        self.SCHEME = {
            "1": "qrsefghabcdttttttttt",
            "2": "abcdijklqrsstttttttt",
            "3": "efghijklmnopqrsefght",
            "4": "mnopqrsmnopqrsmnopqr",
            "5": "abcdefghijklmnopqrst",
        }

        self.SCHEME_LEN = len(self.SCHEME["1"])
...
# <helpers>--------------------------------------------------------------------------------------------------------------------
    ...
    def _draw(self, tokenId):
        mod = (self.tokenIdToSeed[tokenId] % 13) + self.SCHEME_LEN
        output = []
    ...
    def _wrapToSVG(self, tokenId):
        uri = '<svg xmlns="http://www.w3.org/2000/svg" preserveAspectRatio="xMinYMin meet" viewBox="0 0 400 400"><defs><style>@font-face{font-family:"Penrose";src:url("https://penrose.quasarlabs.xyz/penrose.ttf");}</style></defs><style>.base { fill: #'
        uri += self.tokenIdToColorScheme[tokenId][:6]
        uri += '; font-family: "Penrose", monospace;font-size: 8px;}</style><rect width="100%" height="100%" fill="#'
        uri += self.tokenIdToColorScheme[tokenId][6:]
        uri += '" />'
